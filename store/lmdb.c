...
{
  return g_new0(struct LmdbStoreData, 1);
}
...
static int lmdb_get_write_txn(struct LmdbStoreData *sdata)
{
  ...
  if (rc == MDB_SUCCESS)
    ...
    log_debug2("mdb_txn_begin: %s", mdb_strerror(rc));
...
static StoreHandle *store_lmdb_open(const char *path, bool create)
{
  ...
  if (rc != MDB_SUCCESS)
  {
    log_debug2("mdb_env_create: %s", mdb_strerror(rc));
  ...
  if (rc != MDB_SUCCESS)
  {
    log_debug2("mdb_env_open: %s", mdb_strerror(rc));
    goto fail_env;
  }
  ...
  if (rc != MDB_SUCCESS)
  {
    log_debug2("mdb_dbi_open: %s", mdb_strerror(rc));
    goto fail_dbi;
  }
...
static void *store_lmdb_fetch(StoreHandle *store, const char *key, size_t klen, size_t *vlen)
{
  ...
  {
    sdata->txn = NULL;
    log_debug2("txn_renew: %s", mdb_strerror(rc));
    return NULL;
  }
  ...
  if (rc != MDB_SUCCESS)
  {
    log_debug2("mdb_get: %s", mdb_strerror(rc));
    return NULL;
  }
                            ...
                            void *value, size_t vlen)
{
  ...
  if (rc != MDB_SUCCESS)
  {
    log_debug2("lmdb_get_write_txn: %s", mdb_strerror(rc));
    return rc;
  }
  rc = mdb_put(sdata->txn, sdata->db, &dkey, &databuf, 0);
  if (rc != MDB_SUCCESS)
  {
    log_debug2("mdb_put: %s", mdb_strerror(rc));
    mdb_txn_abort(sdata->txn);
    sdata->txn_mode = TXN_UNINITIALIZED;
    sdata->txn = NULL;
  }
...
static int store_lmdb_delete_record(StoreHandle *store, const char *key, size_t klen)
{
  ...
  {
    log_debug2("mdb_del: %s", mdb_strerror(rc));
    mdb_txn_abort(sdata->txn);
    sdata->txn_mode = TXN_UNINITIALIZED;
    sdata->txn = NULL;
  }
