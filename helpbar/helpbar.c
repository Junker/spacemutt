...
static int helpbar_binding_observer(struct NotifyCallback *nc)
{
  ...
  log_debug5("binding done, request WA_RECALC");
  return 0;
}
...
static int helpbar_config_observer(struct NotifyCallback *nc)
{
  ...
  struct MuttWindow *win_helpbar = nc->global_data;
  win_helpbar->state.visible = cs_subset_bool(SpaceMutt->sub, "help");

  mutt_window_reflow(win_helpbar->parent);
  log_debug5("config done: '%s', request WA_REFLOW on parent", ev_c->name);
  return 0;
}
...
static int helpbar_window_observer(struct NotifyCallback *nc)
{
  ...
  if (nc->event_subtype == NT_WINDOW_FOCUS)
  {
    ...
    win_helpbar->actions |= WA_RECALC;
    log_debug5("window focus: request WA_RECALC");
    return 0;
  }
  ...
  {
    win_helpbar->actions |= WA_RECALC;
    log_debug5("window state: request WA_RECALC");
  }
  else if (nc->event_subtype == NT_WINDOW_DELETE)
  {
    mutt_color_observer_remove(helpbar_color_observer, win_helpbar);
    notify_observer_remove(SpaceMutt->notify, helpbar_binding_observer, win_helpbar);
    notify_observer_remove(SpaceMutt->sub->notify, helpbar_config_observer, win_helpbar);
...
struct MuttWindow *helpbar_new(void)
{
  ...
  win->state.visible = cs_subset_bool(SpaceMutt->sub, "help");
  ...
  notify_observer_add(SpaceMutt->notify, NT_BINDING, helpbar_binding_observer, win);
  notify_observer_add(SpaceMutt->sub->notify, NT_CONFIG, helpbar_config_observer, win);
