...
{
  return g_new0(struct HeaderCache, 1);
}
                                ...
                                hcache_namer_t namer, bool create)
{
  ...
  const char *const c_header_cache_backend = cs_subset_string(SpaceMutt->sub, "header_cache_backend");
...
#ifdef USE_HCACHE_COMPRESSION
  const char *const c_header_cache_compress_method = cs_subset_string(SpaceMutt->sub, "header_cache_compress_method");
  if (c_header_cache_compress_method)
  {
    hc->compr_ops = compress_get_ops(c_header_cache_compress_method);

    const short c_header_cache_compress_level = cs_subset_number(SpaceMutt->sub, "header_cache_compress_level");
