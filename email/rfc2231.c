 ...
 */
static void purge_empty_parameters(ParameterList *pl)
{
  for (GList *np = pl->head; np != NULL;)
  {
    GList* next = np->next;
    struct Parameter *p = np->data;
    if (!p->attribute || !p->value)
    {
      g_queue_delete_link(pl, np);
      mutt_param_free(&p);
    }
    np = next;
  }
...
{
  return g_new0(struct Rfc2231Parameter, 1);
}
 ...
 */
static void join_continuations(ParameterList *pl, struct Rfc2231Parameter *par)
{
...
void rfc2231_decode_parameters(ParameterList *pl)
{
  ...
  const bool c_rfc2047_parameters = cs_subset_bool(SpaceMutt->sub, "rfc2047_parameters");
  ...
  for (GList *np = pl->head; np != NULL;)
  {
    ...
    if (!s)
    {
      ...
      if (c_rfc2047_parameters && p->value && strstr(p->value, "=?"))
      {
        rfc2047_decode(&p->value);
      }
      else if (!strlist_is_empty(c_assumed_charset))
      {
        mutt_ch_convert_nonmime_string(c_assumed_charset, c_charset, &p->value);
      }
    }
    else if (s[1] == '\0')
    {
      ...
      s = get_charset(p->value, charset, sizeof(charset));
      decode_one(p->value, s);
      mutt_ch_convert_string(&p->value, charset, c_charset, MUTT_ICONV_HOOK_FROM);
...
size_t rfc2231_encode_string(ParameterList *head, const char *attribute, char *value)
{
  ...
  while (*cur)
  {
    ...
    g_queue_push_tail(head, current);
    count++;
