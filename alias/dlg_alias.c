...
static int alias_window_observer(struct NotifyCallback *nc)
{
  ...
  notify_observer_remove(SpaceMutt->notify, alias_alias_observer, menu);
  notify_observer_remove(SpaceMutt->sub->notify, alias_config_observer, menu);
...
static struct MuttWindow *alias_dialog_new(struct AliasMenuData *mdata)
{
  ...
  notify_observer_add(SpaceMutt->notify, NT_ALIAS, alias_alias_observer, menu);
  notify_observer_add(SpaceMutt->sub->notify, NT_CONFIG, alias_config_observer, menu);
  ...
  if (buf_at(buf, 0) != '\0')
  {
    for (GList *np = Aliases->head; np != NULL; np = np->next)
    {
      ...
      if (alias->name && mutt_strn_equal(alias->name, buf_string(buf), buf_len(buf)))
      {
        ...
        {
          mutt_str_copy(bestname, alias->name,
                        MIN(mutt_str_len(alias->name) + 1, sizeof(bestname)));
        }
        else
        {
          int i;
          for (i = 0; alias->name[i] && (alias->name[i] == bestname[i]); i++)
            ; // do nothing
