...
static void expand_aliases_r(AddressList *al, GSList **expn)
{
  ...
  const bool c_use_domain = cs_subset_bool(SpaceMutt->sub, "use_domain");
  if (c_use_domain && (fqdn = mutt_fqdn(true, SpaceMutt->sub)))
  {
...
{
  const char *const c_config_charset = cs_subset_string(SpaceMutt->sub, "config_charset");
...
bool mutt_addr_is_user(const struct Address *addr)
{
  ...
  const char *fqdn = mutt_fqdn(false, SpaceMutt->sub);
  ...
  fqdn = mutt_fqdn(true, SpaceMutt->sub);
  if (string_is_address(buf_string(addr->mailbox), Username, fqdn))
  {
...
void alias_free(struct Alias **ptr)
{
  ...
  notify_send(SpaceMutt->notify, NT_ALIAS, NT_ALIAS_DELETE, &ev_a);
