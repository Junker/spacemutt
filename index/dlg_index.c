...
static void update_index_threaded(struct MailboxView *mv, enum MxStatus check, int oldcount)
{
  ...
  {
    save_new = g_malloc(num_new * sizeof(struct Email *));
                           ...
                           struct IndexSharedData *shared, bool read_only)
{
  ...
  {
    struct MailboxView *mv = mview_new(m, SpaceMutt->notify);
...
void mutt_draw_statusline(struct MuttWindow *win, int max_cols, const char *buf, size_t buflen)
{
  ...
  do
  {
    ...
    for (GSList *np = regex_colors_get_list(MT_COLOR_STATUS)->head; np != NULL; np = np->next)
    {
      struct RegexColor *cl = np->data;
      ...
      {
        chunks++;
        syntax = g_realloc(syntax, chunks * sizeof(struct StatusSyntax));
      }
...
struct Mailbox *dlg_index(struct MuttWindow *dlg, struct Mailbox *m_init)
{
  ...
  index_adjust_sort_threads(SpaceMutt->sub);

  struct IndexSharedData *shared = dlg->wdata;
  index_shared_data_set_mview(shared, mview_new(m_init, SpaceMutt->notify));
