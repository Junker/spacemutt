...
static int ibar_window_observer(struct NotifyCallback *nc)
{
  ...
  else if (nc->event_subtype == NT_WINDOW_DELETE)
  {
    ...
    notify_observer_remove(SpaceMutt->sub->notify, ibar_config_observer, win_ibar);
...
{
  struct IBarPrivateData *ibar_data = g_new0(struct IBarPrivateData, 1);
                            ...
                            struct IndexPrivateData *priv)
{
  ...
  notify_observer_add(SpaceMutt->sub->notify, NT_CONFIG, ibar_config_observer, win_ibar);
