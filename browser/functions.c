...
{
  bool_str_toggle(SpaceMutt->sub, "imap_list_subscribed", NULL);
...
static int op_browser_view_file(struct BrowserPrivateData *priv, int op)
{
  ...
  else
  {
    ...
    struct Body *b = mutt_make_file_attach(buf_string(path), SpaceMutt->sub);
    if (b)
    {
...
static int op_enter_mask(struct BrowserPrivateData *priv, int op)
{
  ...
  int rc = cs_subset_str_string_set(SpaceMutt->sub, "mask", buf_string(buf), errmsg);
  buf_pool_release(&buf);
...
static int op_sort(struct BrowserPrivateData *priv, int op)
{
  ...
  cs_subset_str_native_set(SpaceMutt->sub, "sort_browser", sort, NULL);
  browser_sort(&priv->state);
  browser_highlight_default(&priv->state, priv->menu);
  menu_queue_redraw(priv->menu, MENU_REDRAW_FULL);
  return FR_SUCCESS;
