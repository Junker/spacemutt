                                    ...
                                    intptr_t data, struct Buffer *err)
{
  ...
  else
  {
    ...
    PatternList *pat = mutt_pattern_comp(mv_cur, menu, pattern, MUTT_PC_NO_FLAGS, err);
    if (!pat)
    {
      FREE(&pattern);
...
void mutt_score_message(struct Mailbox *m, struct Email *e, bool upd_mbox)
{
  ...
  {
    if (mutt_pattern_exec(tmp->pat->data, MUTT_MATCH_FULL_ADDRESS, NULL, e, &cache) > 0)
    {
  ...
  const short c_score_threshold_delete = cs_subset_number(SpaceMutt->sub, "score_threshold_delete");
  const short c_score_threshold_flag = cs_subset_number(SpaceMutt->sub, "score_threshold_flag");
  const short c_score_threshold_read = cs_subset_number(SpaceMutt->sub, "score_threshold_read");
  ...
  while (MoreArgs(s))
  {
    ...
    if (mutt_str_equal("*", buf->data))
    {
      for (tmp = ScoreList; tmp;)
      {
        last = tmp;
        tmp = tmp->next;
        mutt_patternlist_free_full(g_steal_pointer(&last->pat));
        FREE(&last);
      }
      ScoreList = NULL;
    }
      ...
      for (tmp = ScoreList; tmp; last = tmp, tmp = tmp->next)
      {
        if (mutt_str_equal(buf->data, tmp->str))
        {
          if (last)
            ...
            ScoreList = tmp->next;
          mutt_patternlist_free_full(g_steal_pointer(&tmp->pat));
          FREE(&tmp);
