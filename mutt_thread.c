...
{
  struct ThreadsContext *tctx = g_new0(struct ThreadsContext, 1);
...
void mutt_draw_tree(struct ThreadsContext *tctx)
{
  ...
  pfx = g_malloc((width * max_depth) + 2);
  arrow = g_malloc((width * max_depth) + 2);
  const bool c_hide_limited = cs_subset_bool(SpaceMutt->sub, "hide_limited");
  const bool c_hide_missing = cs_subset_bool(SpaceMutt->sub, "hide_missing");
  while (tree)
  {
    if (depth != 0)
    {
      ...
      if (tree->visible)
      {
        ...
        myarrow[width + 1] = 0;
        new_tree = g_malloc(((size_t) depth * width) + 2);
        if (start_depth > 1)
        {
...
static void mutt_sort_subthreads(struct ThreadsContext *tctx, bool init)
{
  ...
  array_size = 256;
  array = g_malloc0_n(array_size, sizeof(struct MuttThread *));
  while (true)
  {
    ...
    while (!thread->next)
    {
      ...
      if (thread->prev && (thread->parent ? thread->parent->sort_children : sort_top))
      {
        ...
        {
          if (i >= array_size)
            array = g_realloc(array, (array_size *= 2) * sizeof(struct MuttThread *));
...
void mutt_sort_threads(struct ThreadsContext *tctx, bool init)
{
  ...
  for (i = 0; i < m->msg_count; i++)
  {
    ...
    else
    {
      ...
      else
      {
        ...
        thread = g_new0(struct MuttThread, 1);
        thread->message = e;
        thread->check_subject = true;
        e->thread = thread;
  ...
  for (i = 0; i < m->msg_count; i++)
  {
    ...
    while (true)
    {
      ...
      {
        tnew = g_new0(struct MuttThread, 1);
