...
static void mh_seq_alloc(struct MhSequences *mhs, int i)
{
  ...
  mhs->flags = g_realloc(mhs->flags, sizeof(mhs->flags[0]) * (newmax + 1));
  while (j <= newmax)
    mhs->flags[j++] = 0;
...
void mh_seq_add_one(struct Mailbox *m, int n, bool unseen, bool flagged, bool replied)
{
  ...
  const char *const c_mh_seq_unseen = cs_subset_string(SpaceMutt->sub, "mh_seq_unseen");
  const char *const c_mh_seq_replied = cs_subset_string(SpaceMutt->sub, "mh_seq_replied");
  const char *const c_mh_seq_flagged = cs_subset_string(SpaceMutt->sub, "mh_seq_flagged");
