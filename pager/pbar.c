...
static int pbar_window_observer(struct NotifyCallback *nc)
{
  ...
  else if (nc->event_subtype == NT_WINDOW_DELETE)
  {
    ...
    notify_observer_remove(SpaceMutt->sub->notify, pbar_config_observer, win_pbar);
...
{
  struct PBarPrivateData *pbar_data = g_new0(struct PBarPrivateData, 1);
...
struct MuttWindow *pbar_new(struct IndexSharedData *shared, struct PagerPrivateData *priv)
{
  ...
  notify_observer_add(SpaceMutt->sub->notify, NT_CONFIG, pbar_config_observer, win_pbar);
