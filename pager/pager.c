...
static int pager_repaint(struct MuttWindow *win)
{
  ...
  if ((priv->redraw & PAGER_REDRAW_FLOW) || repopulate)
  {
    if (!(priv->pview->flags & MUTT_PAGER_RETWINCH))
    {
      ...
      for (int i = 0; i < priv->lines_max; i++)
      {
        ...
        priv->lines[i].syntax = g_realloc(priv->lines[i].syntax, sizeof(struct TextSyntax));
  ...
  if ((priv->redraw & PAGER_REDRAW_PAGER) || (priv->top_line != priv->old_top_line))
  {
    ...
    const bool c_tilde = cs_subset_bool(SpaceMutt->sub, "tilde");
...
static int pager_window_observer(struct NotifyCallback *nc)
{
  ...
  notify_observer_remove(SpaceMutt->sub->notify, pager_config_observer, win_pager);
  notify_observer_remove(SpaceMutt->notify, pager_global_observer, win_pager);
                                    ...
                                    struct PagerPrivateData *priv)
{
  ...
  notify_observer_add(SpaceMutt->sub->notify, NT_CONFIG, pager_config_observer, win);
  notify_observer_add(SpaceMutt->notify, NT_GLOBAL, pager_global_observer, win);
