                          ...
                          int cnt, PagerFlags flags, int special, struct AnsiColor *ansi)
{
  ...
  {
    const bool c_markers = cs_subset_bool(SpaceMutt->sub, "markers");
    if (!cnt && c_markers)
    {
...
{
  bool is_quote = false;
  const struct Regex *c_smileys = cs_subset_regex(SpaceMutt->sub, "smileys");
  regmatch_t pmatch_internal[1] = { 0 };

  if (!pmatch)
    pmatch = pmatch_internal;

  const struct Regex *c_quote_regex = cs_subset_regex(SpaceMutt->sub, "quote_regex");
  ...
  do
  {
    ...
    for (GSList *np = head->head; np != NULL; np = np->next)
    {
      ...
      if (!found)
      {
        ...
        {
          lines[line_num].syntax = g_realloc(lines[line_num].syntax,
...
{
  regmatch_t pmatch[1] = { 0 };
  const bool c_header_color_partial = cs_subset_bool(SpaceMutt->sub, "header_color_partial");
  int offset, i = 0;
  ...
  else if (mutt_str_equal("-- \n", buf) || mutt_str_equal("-- \r\n", buf))
  {
            ...
            (lines[i].cid == MT_COLOR_HEADER)))
    {
      ...
      if (lines[i].syntax_arr_size)
      {
        ...
        lines[line_num].syntax = g_realloc(lines[line_num].syntax, sizeof(struct TextSyntax));
      }
                       ...
                       int *pspecial, int width, AttrColorList *ansi_list)
{
  ...
  const size_t c_wrap = cs_subset_number(SpaceMutt->sub, "wrap");
  ...
  const bool c_allow_ansi = (mode == PAGER_MODE_OTHER) ||
                            cs_subset_bool(SpaceMutt->sub, "allow_ansi");
                 ...
                 struct MuttWindow *win_pager, AttrColorList *ansi_list)
{
  ...
  {
    *lines = g_realloc(*lines, sizeof(struct Line) * (*lines_max += LINES));
    for (ch = *lines_used; ch < *lines_max; ch++)
    {
      memset(&((*lines)[ch]), 0, sizeof(struct Line));
      (*lines)[ch].cid = -1;
      (*lines)[ch].search_arr_size = -1;
      (*lines)[ch].syntax = g_new0(struct TextSyntax, 1);
  ...
  if (flags & (MUTT_SHOWCOLOR | MUTT_TYPES))
  {
    ...
    const short c_toggle_quoted_show_levels = cs_subset_number(SpaceMutt->sub, "toggle_quoted_show_levels");
  ...
  if ((flags & MUTT_SEARCH) && !cur_line->cont_line && (cur_line->search_arr_size == -1))
  {
                   ...
                   (offset ? REG_NOTBOL : 0)) == 0)
    {
      ...
      {
        cur_line->search = g_realloc(cur_line->search,
      ...
      {
        cur_line->search = g_new0(struct TextSyntax, 1);
      }
      pmatch[0].rm_so += offset;
      pmatch[0].rm_eo += offset;
  ...
  const bool c_smart_wrap = cs_subset_bool(SpaceMutt->sub, "smart_wrap");
  if (c_smart_wrap)
  {
    if ((cnt < b_read) && (ch != -1) &&
