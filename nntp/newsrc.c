...
static struct NntpMboxData *mdata_find(struct NntpAccountData *adata, const char *group)
{
  ...
  {
    adata->groups_max *= 2;
    adata->groups_list = g_realloc(adata->groups_list, adata->groups_max * sizeof(mdata));
  }
...
void nntp_newsrc_close(struct NntpAccountData *adata)
{
  ...
  log_debug1("Unlocking %s", adata->newsrc_file);
...
int nntp_newsrc_parse(struct NntpAccountData *adata)
{
  ...
  if (!adata->fp_newsrc)
  {
    log_perror("%s", adata->newsrc_file);
    return -1;
  }

  /* lock it */
  log_debug1("Locking %s", adata->newsrc_file);
  ...
  log_debug1("Parsing %s", adata->newsrc_file);
  ...
  while (st.st_size && fgets(line, st.st_size + 1, adata->fp_newsrc))
  {
    ...
    *p++ = '\0';
    ...
    mdata->newsrc_len = j;
    mdata->newsrc_ent = g_realloc(mdata->newsrc_ent, j * sizeof(struct NewsrcEntry));
    nntp_group_unread_stat(mdata);
    log_debug2("%s", mdata->group);
  }
  FREE(&line);
  return 1;
}
...
void nntp_newsrc_gen_entries(struct Mailbox *m)
{
  ...
  for (int i = 0; i < m->msg_count; i++)
  {
    ...
    if (series)
    {
      ...
      if ((last >= mdata->first_message) && !e->deleted && !e->read)
      {
        ...
        {
          entries *= 2;
          mdata->newsrc_ent = g_realloc(mdata->newsrc_ent, entries * sizeof(struct NewsrcEntry));
        }
  ...
  mdata->newsrc_ent = g_realloc(mdata->newsrc_ent, mdata->newsrc_len * sizeof(struct NewsrcEntry));
...
int nntp_newsrc_update(struct NntpAccountData *adata)
{
  ...
  log_debug1("Updating %s", adata->newsrc_file);
...
static void cache_expand(char *dst, size_t dstlen, struct ConnAccount *cac, const char *src)
{
  ...
  const char *const c_news_cache_dir = cs_subset_path(SpaceMutt->sub, "news_cache_dir");
...
int nntp_add_group(char *line, void *data)
{
  ...
  {
    log_debug2("Can't parse server line: %s", line);
    return 0;
  }
...
static int active_get_cache(struct NntpAccountData *adata)
{
  ...
  log_message(_("Loading list of groups from cache..."));
...
struct HeaderCache *nntp_hcache_open(struct NntpMboxData *mdata)
{
  ...
  const bool c_save_unsubscribed = cs_subset_bool(SpaceMutt->sub, "save_unsubscribed");
...
void nntp_hcache_update(struct NntpMboxData *mdata, struct HeaderCache *hc)
{
  ...
  if (hdata)
  {
    log_debug2("hcache_fetch_email index: %s", hdata);
  ...
  if (!old || (mdata->first_message != first) || (mdata->last_message != last))
  {
    snprintf(buf, sizeof(buf), ANUM_FMT " " ANUM_FMT, mdata->first_message,
             mdata->last_message);
    log_debug2("hcache_store_email index: %s", buf);
...
static int nntp_bcache_delete(const char *id, struct BodyCache *bcache, void *data)
{
  ...
  {
    if (mdata)
      log_debug2("mutt_bcache_del %s", id);
...
static const char *nntp_get_field(enum ConnAccountField field, void *gf_data)
{
  switch (field)
  {
    case MUTT_CA_LOGIN:
    case MUTT_CA_USER:
      return cs_subset_string(SpaceMutt->sub, "nntp_user");
    case MUTT_CA_PASS:
      return cs_subset_string(SpaceMutt->sub, "nntp_pass");
...
struct NntpAccountData *nntp_select_server(struct Mailbox *m, const char *server, bool leave_lock)
{
  ...
  if (!server || (*server == '\0'))
  {
    log_fault(_("No news server defined"));
    return NULL;
  }
  ...
  {
    url_free(&url);
    log_fault(_("%s is an invalid news server specification"), server);
    return NULL;
  }
  ...
  if ((rc >= 0) && c_news_cache_dir)
  {
    ...
    {
      log_fault(_("Can't create %s: %s"), file, strerror(errno));
    }
  ...
  if (rc >= 0)
  {
    const struct Expando *c_newsrc = cs_subset_expando(SpaceMutt->sub, "newsrc");
        ...
        *p = '\0';
        ...
        if (hdata)
        {
          ...
          if (sscanf(hdata, ANUM_FMT " " ANUM_FMT, &first, &last) == 2)
          {
            ...
            {
              mdata->last_cached = last;
              log_debug2("%s last_cached=" ANUM_FMT "", mdata->group, last);
            }
          }
          FREE(&hdata);
        }
                                            ...
                                            struct NntpAccountData *adata, char *group)
{
  ...
  if (mdata->newsrc_ent)
  {
    mdata->newsrc_ent = g_realloc(mdata->newsrc_ent, sizeof(struct NewsrcEntry));
    mdata->newsrc_len = 1;
    mdata->newsrc_ent[0].first = 1;
