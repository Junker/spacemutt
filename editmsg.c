...
static int ev_message(enum EvMessage action, struct Mailbox *m, struct Email *e)
{
  ...
  const unsigned char c_mbox_type = cs_subset_enum(SpaceMutt->sub, "mbox_type");
  cs_subset_str_native_set(SpaceMutt->sub, "mbox_type", MUTT_MBOX, NULL);
  ...
  cs_subset_str_native_set(SpaceMutt->sub, "mbox_type", c_mbox_type, NULL);
