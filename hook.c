                                   ...
                                   intptr_t data, struct Buffer *err)
{
  ...
  const char *const c_default_hook = cs_subset_string(SpaceMutt->sub, "default_hook");
  if (folder_or_mbox)
  {
...
void mutt_select_fcc(struct Buffer *path, struct Email *e)
{
  ...
  if (addr_hook(path, MUTT_FCC_HOOK, NULL, e) != 0)
  {
    ...
    const bool c_save_name = cs_subset_bool(SpaceMutt->sub, "save_name");
    const bool c_force_name = cs_subset_bool(SpaceMutt->sub, "force_name");
