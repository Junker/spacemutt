...
static void window_notify(struct MuttWindow *win)
{
  ...
  log_notify("NT_WINDOW_STATE: %s, %p", mutt_window_win_name(win),
             (void *) win);
...
{
  struct MuttWindow *win = g_new0(struct MuttWindow, 1);
...
void mutt_window_free(struct MuttWindow **ptr)
{
  ...
  log_notify("NT_WINDOW_DELETE: %s, %p", mutt_window_win_name(win),
             (void *) win);
...
void mutt_window_add_child(struct MuttWindow *parent, struct MuttWindow *child)
{
  ...
  log_notify("NT_WINDOW_NEW: %s, %p", mutt_window_win_name(child),
             (void *) child);
...
struct MuttWindow *window_set_focus(struct MuttWindow *win)
{
  ...
  log_notify("NT_WINDOW_FOCUS: %s, %p", mutt_window_win_name(win),
             (void *) win);
  struct EventWindow ev_w = { win, WN_NO_FLAGS };
