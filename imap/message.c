...
static char *msg_parse_flags(struct ImapHeader *h, char *s)
{
  ...
  while (*s && (*s != ')'))
  {
    ...
    {
      s += plen;
      edata->old = cs_subset_bool(SpaceMutt->sub, "mark_old");
    }
    else
    {
      char ctmp;
                                          ...
                                          unsigned int msn_end, unsigned int *fetch_msn_end)
{
  ...
  const long c_imap_fetch_chunk_size = cs_subset_long(SpaceMutt->sub, "imap_fetch_chunk_size");
                                  ...
                                  unsigned int *maxuid, bool initial_download)
{
  ...
  const char *const c_imap_headers = cs_subset_string(SpaceMutt->sub, "imap_headers");
  if (c_imap_headers)
  {
    buf_addch(hdr_list, ' ');
...
bool imap_msg_open(struct Mailbox *m, struct Message *msg, struct Email *e)
{
  ...
  const bool c_imap_peek = cs_subset_bool(SpaceMutt->sub, "imap_peek");
