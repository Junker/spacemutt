...
static int tunnel_socket_open(struct Connection *conn)
{
  ...
  struct TunnelSockData *tunnel = g_new(struct TunnelSockData, 1);
  conn->sockdata = tunnel;

  const char *const c_tunnel = cs_subset_string(SpaceMutt->sub, "tunnel");
   ...
   * conn/connection.h */
  const bool c_tunnel_is_secure = cs_subset_bool(SpaceMutt->sub, "tunnel_is_secure");
