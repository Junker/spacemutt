...
void mutt_actx_add_attach(struct AttachCtx *actx, struct AttachPtr *attach)
{
  ...
  {
    actx->idxmax += grow;
    actx->idx = g_realloc(actx->idx, actx->idxmax * sizeof(struct AttachPtr *));
    actx->v2r = g_realloc(actx->v2r, actx->idxmax * sizeof(short));
...
void mutt_actx_ins_attach(struct AttachCtx *actx, struct AttachPtr *attach, int aidx)
{
  ...
  {
    actx->idxmax += 5;
    actx->idx = g_realloc(actx->idx, sizeof(struct AttachPtr *) * actx->idxmax);
    actx->v2r = g_realloc(actx->v2r, sizeof(short) * actx->idxmax);
...
void mutt_actx_add_fp(struct AttachCtx *actx, FILE *fp_new)
{
  ...
  {
    actx->fp_max += 5;
    actx->fp_idx = g_realloc(actx->fp_idx, sizeof(FILE *) * actx->fp_max);
...
void mutt_actx_add_body(struct AttachCtx *actx, struct Body *b)
{
  ...
  {
    actx->body_max += 5;
    actx->body_idx = g_realloc(actx->body_idx, sizeof(struct Body *) * actx->body_max);
