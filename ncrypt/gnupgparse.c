  ...
  *is_subkey = false;
  ...
  for (p = buf; p; p = pend)
  {
    ...
    switch (field)
    {
      ...
      case 4: /* pubkey algo */
      {
        log_debug2("pubkey algorithm: %s", p);
      ...
      case 10: /* name             */
      {
        ...
        log_debug2("user ID: %s", NONULL(p));

        uid = g_new0(struct PgpUid, 1);
        fix_uid(p);
      ...
      case 12: /* key capabilities */
        log_debug2("capabilities info: %s", p);
