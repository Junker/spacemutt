...
const char *pop_get_field(enum ConnAccountField field, void *gf_data)
{
  switch (field)
  {
    case MUTT_CA_LOGIN:
    case MUTT_CA_USER:
      return cs_subset_string(SpaceMutt->sub, "pop_user");
    case MUTT_CA_PASS:
      return cs_subset_string(SpaceMutt->sub, "pop_pass");
    case MUTT_CA_OAUTH_CMD:
      return cs_subset_string(SpaceMutt->sub, "pop_oauth_refresh_command");
    case MUTT_CA_HOST:
    default:
      return NULL;
  }
                   ...
                   struct Progress *progress, pop_fetch_t callback, void *data)
{
  ...
  char *inbuf = g_malloc(sizeof(buf));
  ...
  while (true)
  {
    ...
    inbuf = g_realloc(inbuf, lenbuf + sizeof(buf));
  }
...
int pop_reconnect(struct Mailbox *m)
{
  ...
  while (true)
  {
    ...
    if (query_quadoption(_("Connection lost. Reconnect to POP server?"),
                         SpaceMutt->sub, "pop_reconnect") != MUTT_YES)
    {
      return -1;
    }
  }
