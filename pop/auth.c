...
static enum PopAuthRes pop_auth_apop(struct PopAccountData *adata, const char *method)
{
  ...
  g_checksum_update(checksum, (const guchar *)adata->timestamp, -1);
  g_checksum_update(checksum, (const guchar *)adata->conn->account.pass, -1);
  strcpy(hash, g_checksum_get_string(checksum));
...
static enum PopAuthRes pop_auth_oauth(struct PopAccountData *adata, const char *method)
{
  ...
  const char *const c_pop_oauth_refresh_command = cs_subset_string(SpaceMutt->sub, "pop_oauth_refresh_command");
  ...
  int rc = pop_query_d(adata, auth_cmd, strlen(auth_cmd),
                       ...
                       (cs_subset_number(SpaceMutt->sub, "debug_level") < MUTT_SOCK_LOG_LEVEL_FULL) ?
                           "AUTH OAUTHBEARER *\r\n" :
#endif
                           NULL);
  FREE(&auth_cmd);
