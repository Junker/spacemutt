...
static int attach_window_observer(struct NotifyCallback *nc)
{
  ...
  else if (nc->event_subtype == NT_WINDOW_DELETE)
  {
    ...
    notify_observer_remove(SpaceMutt->sub->notify, attach_config_observer, win_attach);
...
{
  struct MuttWindow *win_attach = menu_window_new(MENU_COMPOSE, SpaceMutt->sub);
  ...
  notify_observer_add(SpaceMutt->sub->notify, NT_CONFIG, attach_config_observer, win_attach);
