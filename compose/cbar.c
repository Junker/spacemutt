...
static int cbar_window_observer(struct NotifyCallback *nc)
{
  ...
  else if (nc->event_subtype == NT_WINDOW_DELETE)
  {
    ...
    notify_observer_remove(SpaceMutt->sub->notify, cbar_config_observer, win_cbar);
...
struct MuttWindow *cbar_new(struct ComposeSharedData *shared)
{
  ...
  notify_observer_add(SpaceMutt->sub->notify, NT_CONFIG, cbar_config_observer, win_cbar);
